...
namespace DeserializerServer
{
    public class DeserializerServerWindow : EditorTcpServer
    {
        [MenuItem("Window/Deserializer Server")]
        public static void ShowWindow()
        {
            GetWindow<DeserializerServerWindow>("Deserializer Server");
        }

        public override Task<string> ProcessRequest(string request)
        {
            // request is relative path to JSON file starts from Assets/
            string fullPath = System.IO.Path.Combine(
                Application.dataPath,
                request.Replace("Assets/", "")
            );
            ...
            try
            {
                Deserializer<Sensory, Actions>.ReadNodeJson(json);
                return Task.FromResult("Deserialization Successful");
            }
            catch (DeserializationException e)
            {
                return Task.FromResult($"Deserialization Error: {e.Message}");
            }
            catch (Exception e)
            {
    ...
    public abstract class EditorTcpServer : EditorWindow
    {
        ...
        protected virtual void OnGUI()
        {
            ...
            port = EditorGUILayout.IntField("Port", 5001);
