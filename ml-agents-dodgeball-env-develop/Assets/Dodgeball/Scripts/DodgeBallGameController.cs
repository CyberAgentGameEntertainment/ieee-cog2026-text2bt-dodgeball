...
public class DodgeBallGameController : MonoBehaviour
{
    ...
    // Event that fires when a game ends, passes the winning team ID
    ...
    {
        get { return CurrentSceneType != SceneType.Training; }
    }
    ...
    private GameModeType getCurrentGameMode()
    {
        ...
        GameModeType newGameMode =
            isCTFparam > 0.5f ? GameModeType.CaptureTheFlag : GameModeType.Elimination;
        return newGameMode;
    }
    ...
    public void PlayerWasHit(DodgeBallAgent hit, DodgeBallAgent thrower)
    {
        ...
        if (hit.HitPointsRemaining == 1) //FINAL HIT
        {
            ...
            else if (GameMode == GameModeType.Elimination)
            {
                ...
                if (
                    m_NumberOfBluePlayersRemaining == 0
                    || m_NumberOfPurplePlayersRemaining == 0
                    || hit.gameObject == PlayerGameObject
                )
                {
                    ThrowAgentGroup.AddGroupReward(
                        2.0f - m_TimeBonus * (m_ResetTimer / MaxEnvironmentSteps)
                    );
    ...
    public void FlagWasBroughtHome(DodgeBallAgent agent)
    {
        if (m_FlagsAtBase[agent.teamID] || !FlagMustBeHomeToScore)
        {
            ...
            WinAgentGroup.AddGroupReward(
                2.0f - m_TimeBonus * (float)m_ResetTimer / MaxEnvironmentSteps
            );
    ...
    private void GetAllParameters()
    {
        ...
        EliminationHitBonus = m_EnvParameters.GetWithDefault(
            "elimination_hit_reward",
            EliminationHitBonus
        );
    }
    ...
    void ResetScene()
    {
        ...
        for (int ballNum = 0; ballNum < AllBallsList.Count; ballNum++)
        {
            ...
            var spawnPosition =
                BallSpawnPositions[ballSpawnNum].position
                + Random.insideUnitSphere * BallSpawnRadius;
        ...
        foreach (var item in Team0Players)
        {
            ...
            var agentBt = item.Agent.GetComponent<AgentBt>();
            if (agentBt != null)
            {
                agentBt.ResetStates();
            }
