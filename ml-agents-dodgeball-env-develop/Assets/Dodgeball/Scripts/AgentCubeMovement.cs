    ...
    public class AgentCubeMovement : MonoBehaviour
    {
            ...
            rb.MoveRotation(
                rb.rotation
                    * Quaternion.AngleAxis(
                        Mathf.DeltaAngle(smoothYawOld, m_SmoothYaw),
                        transform.up
                    )
            );
        }
        ...
        void FixedUpdate()
        {
            ...
            if (m_Agent)
            {
                ...
                m_Agent.disableInputCollectionInHeuristicCallback =
                    allowHumanInputAndDisableAgentHeuristicInput;
            }
        ...
        public void RotateTowards(Vector3 dir, float maxRotationRate = 1)
        {
            if (dir != Vector3.zero)
            {
                ...
                var smoothedRot = Quaternion.RotateTowards(
                    rb.rotation,
                    rot,
                    maxRotationRate * Time.deltaTime
                );
        ...
        public void RunOnGround(Vector3 dir)
        {
            ...
            else
            {
                if (AnimateBodyMesh)
                {
                    bodyMesh.localPosition =
                        Vector3.zero
                        + Vector3.up
                            * walkingAnimScale
                            * walkingBounceCurve.Evaluate(m_animateBodyMeshCurveTimer);
        ...
        {
            rb.AddForce(Vector3.down * agentFallingSpeed, ForceMode.Acceleration);
        }
    }
}
