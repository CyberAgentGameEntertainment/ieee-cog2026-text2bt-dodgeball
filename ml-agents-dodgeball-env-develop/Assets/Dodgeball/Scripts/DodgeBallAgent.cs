...
public class DodgeBallAgent : Agent
{
    [Header("TEAM")]
    public int teamID;
    public AgentCubeMovement m_CubeMovement;
    ...
    [Header("SHIELD")]
    public ShieldController AgentShield;
    ...
    [Header("OTHER")]
    public bool m_PlayerInitialized;
    ...
    public void ResetAgent()
    {
        ...
        if (
            m_GameController.CurrentSceneType == DodgeBallGameController.SceneType.Game
            || m_GameController.CurrentSceneType == DodgeBallGameController.SceneType.Movie
        )
        {
    ...
    {
        m_DashCoolDownReady =
            m_CubeMovement.dashCoolDownTimer > m_CubeMovement.dashCoolDownDuration;
        if (StepCount % 5 == 0)
        {
    ...
    private float[] GetOtherAgentData(DodgeBallGameController.PlayerInfo info)
    {
        ...
        otherAgentdata[0] =
            (float)info.Agent.HitPointsRemaining / (float)NumberOfTimesPlayerCanBeHit;
    ...
    public bool Stunned
    {
        ...
        set
        {
            if (value)
            {
                ...
                if (
                    m_GameController.GameMode == DodgeBallGameController.GameModeType.CaptureTheFlag
                )
                {
                    AgentRb.mass = 100000;
                }
    ...
    private void OnCollisionEnter(Collision col)
    {
        ...
        if (!db)
        {
            if (m_GameController.GameMode == DodgeBallGameController.GameModeType.CaptureTheFlag)
            {
                ...
                if (
                    col.gameObject.tag == "purpleFlag" && teamID == 0
                    || col.gameObject.tag == "blueFlag" && teamID == 1
                )
                {
                    m_GameController.FlagWasTaken(this);
                }
                else if (
                    col.gameObject.tag == "purpleFlag" && teamID == 1
                    || col.gameObject.tag == "blueFlag" && teamID == 0
                )
                {
